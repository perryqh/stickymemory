...
describe PlanBuilder do
  describe "plan" do
    def test_number_of_verses(book_start_num, book_end_num, duration, expected_number)
      PlanBuilder.new(Plan.new(:days_duration => duration, :start_bible_book => book_by_num(book_start_num), :end_bible_book => book_by_num(book_end_num))).send(:verses_between).should eql expected_number
    end
    def test_number_of_verses_per_day(book_start_num, book_end_num, duration, expected_number)
      PlanBuilder.new(Plan.new(:days_duration => duration, :start_bible_book => book_by_num(book_start_num), :end_bible_book => book_by_num(book_end_num))).send(:verses_per_day).should eql expected_number
    end
    ...
    it "should build proper 11 day plan" do
      plan = Plan.create(:user => user = User.make, :start_bible_book => book_by_num(1), :end_bible_book => book_by_num(4), :days_duration => 11)
      plan.start_bible_book.should eql book_by_num(1)
      plan.end_bible_book.should eql book_by_num(4)
      plan.user.should eql user
    ...
    it "should build 1 day play" do
      plan = Plan.create(:user => user = User.make, :start_bible_book => book_by_num(1), :end_bible_book => book_by_num(1), :days_duration => 1)
      plan.start_bible_book.should eql book_by_num(1)
    ...
    it "should build proper 1 year plan" do
      plan = Plan.create(:user => user = User.make, :start_bible_book => book_by_num(1), :end_bible_book => book_by_num(BibleBook.last.book_num), :days_duration => 365)
      plan.start_bible_book.should eql book_by_num(1)
      plan.end_bible_book.should eql book_by_num(BibleBook.last.book_num)
      plan.user.should eql user
      plan.start_date.should eql DateTime.now.to_date
  ...
  describe 'next coordinate' do
    before(:each) do
      @plan = Plan.new(:start_bible_book => book_by_num(1), :end_bible_book => book_by_num(2), :days_duration => 30)
      @plan_builder = PlanBuilder.new(@plan)
      @book = BibleBook.first
      @next_book = BibleBook.find_by_book_num(@book.book_num + 1)
      @plan_builder.stubs(:verses_per_day => 10)
    ...
    it "should jump to previous end of chapter" do
      @plan_builder.stubs(:verses_per_day => 20)
